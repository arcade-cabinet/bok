...
const BIOMES = [
  ...
  { id: 'volcanic', name: 'Cinderpeak Caldera', desc: 'Rivers of lava through obsidian fields', icon: '🌋', sky: '#CC4125' },
  { id: 'swamp', name: 'Rothollow Marsh', desc: 'Murky waters and poisonous fog', icon: '🌿', sky: '#6B8E5E' },
  { id: 'crystal', name: 'Prismatic Depths', desc: 'Crystalline caverns of refracted light', icon: '💎', sky: '#9B59B6' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden">
      ...
      {['top-4 left-4', 'top-4 right-4 rotate-90', 'bottom-4 left-4 -rotate-90', 'bottom-4 right-4 rotate-180'].map((pos, i) => (
        <div key={i} className={`fixed ${pos} opacity-30 pointer-events-none`}>
          <svg width="50" height="50" viewBox="0 0 60 60" fill="none">
            <path d="M2 2 L2 25 Q2 2 25 2 L2 2 M5 5 L5 20 Q5 5 20 5 L5 5" stroke="#c4a572" strokeWidth="1.5" fill="none" />
          </svg>
        </div>
      ))}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-4 max-w-lg mx-auto w-full">
        ...
        <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] }} className="text-center mb-12">
          ...
          <p className="text-lg tracking-[0.3em] uppercase" style={{ fontFamily: 'Crimson Text, Georgia, serif', color: '#a89574' }}>
            The Builder's Tome
          </p>
        ...
        {!showNewGame ? (
          <div className="w-full space-y-3">
            <motion.button variants={menuItem(0)} initial="hidden" animate="visible" onClick={() => setShowNewGame(true)}
              ...
              style={{ fontFamily: 'Crimson Text, Georgia, serif', color: '#d4c5a0', background: 'rgba(58,40,32,0.8)', borderColor: 'rgba(139,115,85,0.6)', letterSpacing: '0.05em' }}>
              ✒ Pen New Tale
            </motion.button>
            ...
            <motion.button variants={menuItem(2)} initial="hidden" animate="visible"
              ...
              style={{ fontFamily: 'Crimson Text, Georgia, serif', color: '#a89574', background: 'rgba(45,31,23,0.5)', borderColor: 'rgba(139,115,85,0.4)', letterSpacing: '0.05em' }}>
              ⚙ Inscriptions
            </motion.button>
          </div>
        ) : (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full space-y-4">
            <h2 className="text-2xl text-center mb-4" style={{ fontFamily: 'Cinzel, Georgia, serif', color: '#c4a572' }}>
              Choose Your Chapter
            </h2>
            ...
            <div className="grid grid-cols-2 gap-2 max-h-[40vh] overflow-y-auto pr-1">
              {BIOMES.map(b => (
                <button key={b.id} onClick={() => setSelectedBiome(b.id)}
                  ...
                  <div className="text-sm font-bold" style={{ fontFamily: 'Cinzel, Georgia, serif', color: '#d4c5a0' }}>{b.name}</div>
                  <div className="text-xs italic" style={{ color: '#8b7355' }}>{b.desc}</div>
                </button>
              ))}
            </div>
            ...
            {/* Seed input */}
            <div>
              <label className="text-sm block mb-1" style={{ fontFamily: 'Cinzel, Georgia, serif', color: '#c4a572' }}>World Seed</label>
              <div className="flex gap-2">
                <input value={seed} onChange={e => setSeed(e.target.value)} placeholder="Let fate decide..."
                  className="flex-1 px-3 py-2 rounded-md border text-sm"
                  style={{ background: 'rgba(26,18,12,0.7)', borderColor: 'rgba(107,84,68,0.6)', color: '#d4c5a0', fontFamily: 'monospace' }} />
                <button onClick={randomSeed} className="px-3 py-2 rounded-md border cursor-pointer"
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleStart}
                ...
                style={{ fontFamily: 'Cinzel, Georgia, serif', color: '#d4c5a0', background: '#5a4738', letterSpacing: '0.05em' }}>
                ✒ Begin Writing
              </button>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
          className="absolute bottom-6 text-center text-xs italic" style={{ color: '#6b5444' }}>
