...
describe('StaminaSystem', () => {
  ...
  it('regenerates stamina when not sprinting', () => {
    const entity = world.spawn(Stamina({ current: 50, max: 100, regenRate: 20 }));
  ...
  it('clamps stamina to max', () => {
    const entity = world.spawn(Stamina({ current: 95, max: 100, regenRate: 20 }));
  ...
  it('clamps stamina to 0', () => {
    const entity = world.spawn(Stamina({ current: 5, max: 100, regenRate: 20 }));
    world.set(MovementIntent, { dirX: 0, dirZ: 1, sprint: true, jump: false });
